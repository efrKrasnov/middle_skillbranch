...
class User private constructor(
    ...
    private fun encrypt(password: String): String = salt.plus(password).md5()   // don't do that!!!!
            ...
            .run {
                when (size) {
                    2 -> first() to last()
                        ...
                        "Hash must contain salt and hash, current split result $this"
                    )
                }
            }
    }
    ...
    companion object Factory {
        fun makeUser(
            ...
            return when {
                !phone.isNullOrBlank() && hash.isNullOrBlank() -> User(
                    firstName,
                    lastName,
                    rawPhone = phone
                )
                !email.isNullOrBlank() && !password.isNullOrBlank() && hash.isNullOrBlank() -> User(
                    firstName,
                    lastName,
                    email,
                    password
                )
                !hash.isNullOrBlank() && (!phone.isNullOrBlank() || !email.isNullOrBlank()) ->
                    User(
                        firstName,
                        lastName,
                        email = email,
